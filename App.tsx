...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const updateWeaponModel = () => {
            ...
            // --- 3.5 Player Model (for 3rd Person) ---
            ...
            playerModel.current.add(pBody, pHead);
            scene.add(playerModel.current);
            playerModel.current.visible = false;
            ...
            const respawnPlayer = () => {
                ...
                playerLogicalPos.current.set(0, 1.6, 5);
                velocity.set(0, 0, 0);
                updatePlayerHPUI();
            ...
            const onKeyDown = (e: KeyboardEvent) => {
                ...
                if (e.code === 'KeyG') { isThirdPerson.current = !isThirdPerson.current; }
            ...
            const animate = () => {
                ...
                if (isLocked) {
                    ...
                    if (!isDead.current) {
                        ...
                        playerLogicalPos.current.add(camDir.multiplyScalar(-velocity.z * delta));
                        playerLogicalPos.current.add(camSide.multiplyScalar(-velocity.x * delta));
                        playerLogicalPos.current.y += velocity.y * delta;

                        const playerPadding = 0.8;
                        if (playerLogicalPos.current.x > MAP_SIZE - playerPadding) playerLogicalPos.current.x = MAP_SIZE - playerPadding;
                        if (playerLogicalPos.current.x < -MAP_SIZE + playerPadding) playerLogicalPos.current.x = -MAP_SIZE + playerPadding;
                        if (playerLogicalPos.current.z > MAP_SIZE - playerPadding) playerLogicalPos.current.z = MAP_SIZE - playerPadding;
                        if (playerLogicalPos.current.z < -MAP_SIZE + playerPadding) playerLogicalPos.current.z = -MAP_SIZE + playerPadding;

                        if (playerLogicalPos.current.y < 1.6) {
                            velocity.y = 0; playerLogicalPos.current.y = 1.6; moveState.canJump = true; moveState.isAirborne = false;
                        }
                    }
                    ...
                    if (isThirdPerson.current) {
                        ...
                        // Orbit camera: Position behind the player logical position
                        const cameraTargetPos = playerLogicalPos.current.clone().add(backDir.multiplyScalar(4)).add(new THREE.Vector3(0, 1, 0));
                        camera.position.lerp(cameraTargetPos, 0.2);

                        playerModel.current.visible = true;
                        playerModel.current.position.copy(playerLogicalPos.current);
                        playerModel.current.rotation.y = camera.rotation.y;
                        weaponGroup.current.visible = false;
                    } else {
                        camera.position.copy(playerLogicalPos.current);
                        playerModel.current.visible = false;
                        weaponGroup.current.visible = true;
                    }
